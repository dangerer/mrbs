...
$baseurl = new moodle_url('/blocks/mrbs/web/week.php', ['day' => $day, 'month' => $month, 'year' => $year]); // Used as the basis for URLs throughout this file
...
if ($area_list_format == "select") {
    ...
    $this_area_name = $DB->get_field('block_mrbs_area', 'area_name', ['id' => $area]);
    $this_room_name = $DB->get_field('block_mrbs_room', 'room_name', ['id' => $room]);
    $this_room_description = $DB->get_field('block_mrbs_room', 'description', ['id' => $room]);
} else {
...
for ($j = 0; $j <= ($num_of_days - 1); $j++) {
    ...
    $entries = $DB->get_records_select('block_mrbs_entry', $sql, [$room, $pm7[$j], $am7[$j]]);
...
for ($j = 0; $j <= ($num_of_days - 1); $j++) {
    ...
    $dayurl = new moodle_url('/blocks/mrbs/web/day.php', ['year' => userdate($t, "%Y"), 'month' => userdate($t, "%m"),
        ...
        if (!isset($id)) {
            ...
            if ($pview != 1) {
                if (!$allowedtobook) {
                    ...
                    echo "<center>";
                    $editentry = new moodle_url('/blocks/mrbs/web/edit_entry.php', ['room' => $room, 'area' => $area, 'year' => $wyear,
            ...
            $viewentry = new moodle_url('/blocks/mrbs/web/view_entry.php', ['id' => $id, 'area' => $area, 'day' => $wday,
