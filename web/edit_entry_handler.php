...
if (!getWritable($create_by, getUserName())) {
    if ($editunconfirmed) {
        foreach ($rooms as $key => $room) {
            $adminemail = $DB->get_field('block_mrbs_room', 'room_admin_email', ['id' => $room]);
...
foreach ($rooms as $room_id) {
    if ($rep_type != 0 && !empty($reps)) {
        ...
        if (has_capability("block/mrbs:forcebook", $context) and $forcebook) {
            ...
            $tmp = '';
        } elseif ($doublebook and has_capability('block/mrbs:doublebook', $context)) {
            ...
            foreach ($clashingbookings as $clashingbooking) {
                ...
                if (!email_to_user($oldbookinguser, $USER, get_string('doublebookesubject', 'block_mrbs'), get_string('doublebookebody', 'block_mrbs', $langvars))) {
                    email_to_user($DB->get_record('user', ['email' => $mrbs_admin_email]), $USER, get_string('doublebookefailsubject', 'block_mrbs'), get_string('doublebookefailbody', 'block_mrbs', $oldbookinguser->username) . get_string('doublebookebody', 'block_mrbs', $langvars));
                }
            }
        } else {
...
if (empty($err)) {
    ...
    $dayurl = new moodle_url('/blocks/mrbs/web/day.php', ['year' => $year, 'month' => $month, 'day' => $day, 'area' => $area]);
