...
switch ($default_view) {
    case "month":
        $redirect = new moodle_url('/blocks/mrbs/web/month.php', ['year' => $year, 'month' => $month]);
        break;
    case "week":
        $redirect = new moodle_url('/blocks/mrbs/web/week.php', ['year' => $year, 'month' => $month, 'day' => $day]);
        break;
    default:
        $redirect = new moodle_url('/blocks/mrbs/web/day.php', ['day' => $day, 'month' => $month, 'year' => $year]);
}

if (!empty($default_room)) {
    //	$sql = "select area_id from $tbl_room where id=$default_room";
    $res = $DB->get_record('block_mrbs_room', ['id' => $default_room]);
    if (!empty($res)) {
