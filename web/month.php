...
$baseurl = new moodle_url('/blocks/mrbs/web/month.php', ['month' => $month, 'year' => $year]); // Used as basis for URLs throughout this file
...
if ($area_list_format == "select") {
    ...
    $this_area_name = s($DB->get_field('block_mrbs_area', 'area_name', ['id' => $area]));
    $this_room_name = s($DB->get_field('block_mrbs_room', 'room_name', ['id' => $room]));
} else {
...
for ($day_num = 1; $day_num <= $days_in_month; $day_num++) {
    ...
    $entries = $DB->get_records_select('block_mrbs_entry', 'room_id = ? AND start_time <= ? AND end_time > ?', [$room, $midnight_tonight[$day_num], $midnight[$day_num]], 'start_time');
...
$roomdata = $DB->get_record('block_mrbs_room', ['id' => $room]);
...
for ($cday = 1; $cday <= $days_in_month; $cday++) {
    ...
    $dayurl = new moodle_url('/blocks/mrbs/web/day.php', ['year' => $year, 'month' => $month, 'day' => $cday, 'area' => $area]);
    ...
    if (isset($d[$cday]["id"][0])) {
        ...
        for ($i = 0; $i < $n; $i++) {
            ...
            $viewentry_url = new moodle_url('/blocks/mrbs/web/view_entry.php', ['id' => $d[$cday]['id'][$i], 'day' => $cday, 'month' => $month, 'year' => $year]);
    ...
    if ($pview != 1) {
        if (!$allowedtobook) {
            ...
            $editurl = new moodle_url('/blocks/mrbs/web/edit_entry.php', ['room' => $room, 'area' => $area, 'year' => $year, 'month' => $month, 'day' => $cday]);
            if ($enable_periods) {
