...
{ //if values are not passed assume null
    ...
    if ($pview != 1) {
        ...
        $helpurl = new moodle_url('/blocks/mrbs/web/help.php', ['day' => $day, 'month' => $month, 'year' => $year]);

        $adminstr = get_string('admin');
        $adminurl = new moodle_url('/blocks/mrbs/web/admin.php', ['day' => $day, 'month' => $month, 'year' => $year]);
...
HTML2END;
        if (!$userview) {
            if (has_capability("block/mrbs:forcebook", $context)) {
                ...
                echo 'href="edit_entry.php?force=TRUE">' . get_string('addentry', 'block_mrbs') . '</a>
              </td>';
            }
            /*
...
function get_default_room($area)
{
    ...
    $room = $DB->get_records('block_mrbs_room', ['area_id' => $area], 'room_name', 'id', 0, 1);
    if (empty($room)) {
        return 0;
    }
    // Extract the first (and only!) item in the returned array
