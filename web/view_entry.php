...
$thisurl = new moodle_url('/blocks/mrbs/web/view_entry.php', ['day' => $day, 'month' => $month, 'year' => $year, 'id' => $id]);
...
if ($series == 1) {
    ...
    $entry = $DB->get_records('block_mrbs_entry', ['repeat_id' => $id, 'entry_type' => 1], 'start_time', 'id', 0, 1);
    if (empty($entry)) {
        ...
        $entry = $DB->get_records('block_mrbs_entry', ['repeat_id' => $id], 'start_time', 'id', 0, 1);
    }
    ...
    if ($repeat_id != 0) {
        $repeat = $DB->get_record('block_mrbs_repeat', ['id' => $repeat_id]);
        if ($repeat) {
...
<p>
    ...
    if ($canedit || $roomadmin) {
        if (!$series) {
            $editurl = new moodle_url('/blocks/mrbs/web/edit_entry.php', ['id' => $id]);
        ...
        if ($repeat_id || $series) {
            $editurl = new moodle_url('/blocks/mrbs/web/edit_entry.php', ['id' => $id, 'edit_type' => 'series', 'day' => $day, 'month' => $month, 'year' => $year]);
        ...
        if ($repeat_id || $series) {
            $delurl = new moodle_url('/blocks/mrbs/web/del_entry.php', ['id' => $id, 'series' => 1, 'sesskey' => sesskey(), 'day' => $day, 'month' => $month, 'year' => $year]);
