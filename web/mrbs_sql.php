...
function mrbsCheckFree($room_id, $starttime, $endtime, $ignore, $repignore)
{
    ...
    foreach ($entries as $entry) {
        ...
        $viewurl = new moodle_url('/blocks/mrbs/web/view_entry.php', ['id' => $entry->id]);
        $dayurl = new moodle_url('/blocks/mrbs/web/day.php', $param_ymd);
        $weekurl = new moodle_url('/blocks/mrbs/web/week.php', array_merge($param_ymd, ['room' => $room_id]));
        $monthurl = new moodle_url('/blocks/mrbs/web/month.php', array_merge($param_ym, ['room' => $room_id]));
...
function mrbsDelEntry($user, $id, $series, $all, $roomadminoverride = false)
{
    ...
    $repeat_id = $DB->get_field('block_mrbs_entry', 'repeat_id', ['id' => $id]);
    if ($repeat_id < 0) {
        return 0;
    }
    ...
    foreach ($entries as $entry) {
        ...
        $DB->delete_records('block_mrbs_entry', ['id' => $entry->id]);

        $removed++;
    }

    if ($repeat_id > 0 && $DB->count_records('block_mrbs_entry', ['repeat_id' => $repeat_id]) == 0) {
        $DB->delete_records('block_mrbs_repeat', ['id' => $repeat_id]);
    }
...
function mrbsCreateRepeatingEntrys($starttime, $endtime, $rep_type, $rep_enddate, $rep_opt, $room_id, $owner, $name, $type, $description, $rep_num_weeks, $roomchange = false, $oldid = 0)
{
    ...
    $repeatid = 0;
    if ($oldid) {
        $repeatid = $DB->get_field('block_mrbs_entry', 'repeat_id', ['id' => $oldid]);
    }
    ...
    if (empty($reps)) {
        if ($repeatid) {
            ...
            $DB->delete_records('block_mrbs_repeat', ['id' => $repeatid]);
        }
    ...
    if ($ret->id) {
        ...
        if ($repeatid) {
            ...
            $oldids = $DB->get_fieldset_sql('SELECT id FROM {block_mrbs_entry} WHERE repeat_id = ? ORDER BY start_time', [$repeatid]);
        }
        ...
        for ($i = count($reps); $i < count($oldids); $i++) {
            $DB->delete_records('block_mrbs_entry', ['id' => $oldids[$i]]);
        }
    }
    return $ret;
}
...
{
    global $DB;
    return $DB->get_record('block_mrbs_entry', ['id' => $id]);
}
...
function mrbsGetRoomArea($id)
{
    ...
    return $DB->get_field('block_mrbs_room', 'area_id', ['id' => $id]);
}
