...
$thisurl = new moodle_url('/blocks/mrbs/web/del.php', ['day' => $day, 'month' => $month, 'year' => $year, 'type' => $type]);
if ($area) {
...
if ($type == "room") {
    ...
    if ($confirm) {
        // Delete bookings
        $DB->delete_records('block_mrbs_entry', ['room_id' => $room]);

        // Delete the room
        $DB->delete_records('block_mrbs_room', ['id' => $room]);
        ...
        $bookings = $DB->get_records('block_mrbs_entry', ['room_id' => $room]);
        if (!empty($bookings)) {
...
if ($type == "area") {
    ...
    // no rooms. its easier
    $n = $DB->count_records('block_mrbs_room', ['area_id' => $area]);
    if ($n == 0) {
        // OK, nothing there, lets blast it away
        $DB->delete_records('block_mrbs_area', ['id' => $area]);
