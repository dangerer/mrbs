...
$baseurl = new moodle_url('/blocks/mrbs/web/day.php', ['day' => $day, 'month' => $month, 'year' => $year]); // Used as basis for URLs throughout this file
...
if (!empty($area)) {
    ...
    if (empty($rooms)) {
        ...
        echo "<tr><thead class=\"thead-light\"><th width=\"1%\" class=\"text-center align-top\">" . ($enable_periods ? get_string('period', 'block_mrbs') : get_string('time')) . "</th>";

        $room_column_width = (int) (95 / count($rooms));
        $weekurl = new moodle_url('/blocks/mrbs/web/week.php', ['year' => $year, 'month' => $month, 'day' => $day, 'area' => $area]);
        foreach ($rooms as $room) {
        ...
        if (false != $times_right_side) {
            echo "<th class=\"align-top\" width=\"1%\">" . ($enable_periods ? get_string('period', 'block_mrbs') : get_string('time'))
            . "</th>";
        }
        echo "</thead></tr><tbody>\n";
        ...
        for ($t = $starttime; $t <= $endtime; $t += $resolution) {
            ...
            foreach ($rooms as $room) {
                ...
                if (!isset($id)) {
                    ...
                    if ($pview != 1) {
                        if (!$room->allowedtobook) {
                            ...
                            echo "<center>";
                            $editurl = new moodle_url('/blocks/mrbs/web/edit_entry.php', ['room' => $room->id, 'area' => $area, 'year' => $year, 'month' => $month, 'day' => $day]);
                            if ($enable_periods) {
                ...
                for ($i = 0; $i < count($descrs); $i++) {
                    $viewentry = new moodle_url('/blocks/mrbs/web/view_entry.php', ['id' => $ids[$i], 'area' => $area, 'day' => $day, 'month' => $month, 'year' => $year]);
                    if ($descrs[$i] != "") {
