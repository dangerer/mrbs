...
if (!empty($cfg_mrbs->cronfile) && file_exists($cfg_mrbs->cronfile)) {
    if ($mrbs_sessions = fopen($cfg_mrbs->cronfile, 'r')) {
        ...
        $now = time();
        $DB->set_field_select('block_mrbs_entry', 'type', 'M', 'type=\'K\' and start_time > ?', [$now]); // Change old imported (type K) records to temporary type M
        while ($array = fgetcsv($mrbs_sessions)) { //import timetable into mrbs
            ...
            foreach ($weeks as $week) {
                if (($week == 1) and ($date > $now)) {
                    ...
                    $end_time = mrbs_3($date, $csvrow->end_time);
        ...
        if ($mrbsadmin = $DB->get_record('user', ['email' => $cfg_mrbs->admin_email])) {
...
class import
{
  ...
  //==========================================FUNCTIONS==============================================================
    ...
    {
        global $DB;
        if (!$room = $DB->get_record('block_mrbs_room', ['room_name' => $name])) {
    ...
    {
        global $DB;
        if ($DB->get_record('block_mrbs_entry', ['name' => $name, 'start_time' => $time, 'type' => 'L'])) {
            return true;
        } elseif ($record = $DB->get_record('block_mrbs_entry', [
